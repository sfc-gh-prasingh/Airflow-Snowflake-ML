...
def inference_pipeline():
    ...
    def score_data():
        ...
        session.sql("""
            CREATE TABLE IF NOT EXISTS PRANJ.TEST.NEW_FEATURE_DATA AS
            SELECT
        ...
        input_df = session.table("PRANJ.TEST.NEW_FEATURE_DATA")
        predictions = mv.run(input_df, function_name="predict")
        predictions.write.save_as_table("PRANJ.TEST.MONTHLY_PREDICTIONS", mode="overwrite")

        row_count = session.table("PRANJ.TEST.MONTHLY_PREDICTIONS").count()
        session.close()
    ...
    def log_result(score_output: str):
        ...
        session.sql("""
            CREATE TABLE IF NOT EXISTS PRANJ.TEST.INFERENCE_LOG (
        ...
        session.sql(f"""
            INSERT INTO PRANJ.TEST.INFERENCE_LOG (RUN_DATE, STATUS, ROWS_SCORED)
